...
use App\Helpers\Collections\Configs\ConfigFinderCollection;
...
class FindConfig
{
    ...
    public function parentIn($code)
    {
        ...
        return new ConfigFinderCollection($this->key, $codes, $types->toArray());
    }
    ...
    public function name($name)
    {
        ...
        $types = $this->config->defaultWith($this->config->defaultSelects)
            ->whereIn(DB::raw("TRIM(LOWER(config_name))"), $names)
            ->get();

        return new ConfigFinderCollection('config_name', $names, $types->toArray());
    }
}
